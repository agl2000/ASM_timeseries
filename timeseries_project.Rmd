...
model1 <- arima(d1_d12_ln_serie, order = c(2, 0, 0))
model2 <- arima(d1_d12_ln_serie, order = c(0, 0, 4))
model3 <- arima(d1_d12_ln_serie, order = c(2, 0, 4))

# Additional models you might consider:
model4 <- arima(d1_d12_ln_serie, order = c(1, 0, 1))  # AR(1), MA(1)
model5 <- arima(d1_d12_ln_serie, order = c(2, 0, 1))  # AR(2), MA(1)
model6 <- arima(d1_d12_ln_serie, order = c(1, 0, 2))  # AR(1), MA(2)
...
cat("Best model")
best_model <- arima(d1_d12_ln_serie, order = c(2, 0, 4))
...
pred <- predict(best_model, n.ahead = 12)
pr <- ts(c(tail(d1_d12_ln_serie, 1), pred$pred), start = end(d1_d12_ln_serie), freq = 12)
se <- ts(c(0, pred$se), start = end(d1_d12_ln_serie), freq = 12)

# Intervals
tl <- ts(exp(pr - 1.96 * se), start = end(d1_d12_ln_serie), freq = 12)
tu <- ts(exp(pr + 1.96 * se), start = end(d1_d12_ln_serie), freq = 12)
pr <- ts(exp(pr), start = end(d1_d12_ln_serie), freq = 12)

# Plot the prediction
ts.plot(serie, tl, tu, pr, lty = c(1, 2, 2, 1), col = c(1, 4, 4, 2), xlim = end(d1_d12_ln_serie)[1] + c(-2, +2), type = "o", main = paste("Model ARIMA(", paste(c(2, 0, 4), collapse = ","), ")", sep = ""))
abline(v = (end(d1_d12_ln_serie)[1] - 2):(end(d1_d12_ln_serie)[1] + 2), lty = 3, col = 4)
